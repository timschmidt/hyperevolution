...
- `FitnessValue`, `FitnessReport`, `FitnessDirection`, `FitnessComparison`, and
  `ParetoRelation` describe exact scalar, lexicographic, and Pareto ordering.
...
- `SelectionPolicy`, `MutationPolicy`, `CrossoverPolicy`, `HillClimbPolicy`, and
  `SimulatedAnnealingPolicy` record proposal settings without implementing every
  optimizer family.

## Precision Model

Genome values and fitness values use `Real`. Comparison helpers avoid collapsing exact
fitness to primitive floats before ordering. Replay status is explicit, so a candidate
that is promising under a sampled or approximate proposal can remain pending or unknown
until the domain crate certifies it.
...
- scalar, lexicographic, and Pareto fitness reports over `Real` values;
